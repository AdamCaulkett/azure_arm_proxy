...
func (nsg *NetworkSecurityGroup) GetPath() string {
	return fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/%s/%s?api-version=%s", config.BaseURL, *config.SubscriptionIDCred, nsg.createParams.Group, networkSecurityGroupPath, nsg.createParams.Name, microsoftNetworkApiVersion)
}
...
func (nsg *NetworkSecurityGroup) GetCollectionPath(groupName string) string {
	if groupName == "" {
		return fmt.Sprintf("%s/subscriptions/%s/%s?api-version=%s", config.BaseURL, *config.SubscriptionIDCred, networkSecurityGroupPath, microsoftNetworkApiVersion)
	}
	return fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/%s?api-version=%s", config.BaseURL, *config.SubscriptionIDCred, groupName, networkSecurityGroupPath, microsoftNetworkApiVersion)
}
